...
* [How to Run with Gradle](#how-to-run-with-gradle)
  * [targeting an instance of Robert](#targeting-an-instance-of-robert)
...
### targeting an instance of Robert

By default the application is configured to interact with an instance of Robert running on http://localhost:8080.

If you want to override this default, then execute with these additional environment variables, e.g.,

```bash
export REFACTOR_SERVICE_SCHEME=https
export REFACTOR_SERVICE_HOST=robert.dev.jitterbug.io
export REFACTOR_SERVICE_PORT=443
...
```bash
gh repo clone cf-toolsuite/robert-ui
cd robert-ui
...
At this point you'd probably like to interact with robert-ui, huh?  We need to setup port-forwarding, so execute:

```bash
kubectl port-forward service/robert-ui 8080:8080
```
...
Consider this a Quick Start guide to getting `robert-ui` deployed using the [tanzu](https://docs.vmware.com/en/VMware-Tanzu-Application-Platform/1.12/tap/install-tanzu-cli.html) CLI.
...
Edit the file `.tanzu/config/robert-ui.yml`.
...
> Replace the value of `REPLACE_ME` for `REFACTOR_SERVICE_HOST` above with the URL to an instance of `robert`.
...
```bash
tanzu app get robert-ui
tanzu app logs robert-ui --lines -1
```

### Establish a domain binding

```bash
tanzu domain-binding create robert-ui --domain robert-ui.sbx.tpk8s.cloudmonk.me --entrypoint main --port 443
```
